...
#Regression discontinuity with controls and fixed effects (optimal bandwidth)

rd_robust_parks_2_ctrl_fx <- lapply(defo_dist, function(park){
  rdrobust(
    y = park$loss_sum,
    x = park$dist_disc,
    covs = cbind(park$altura_tile_30arc, park$slope, park$roughness, park$prec, 
                 park$sq_1km.1, park$treecover_agg, park$clumps_1, park$buffer_id),
    vce = "nn",
    nnmatch = 3,
    all = T
  )
...
rd_robust_terr_2_ctrl_fx <- lapply(defo_dist_terr, function(park){
  rdrobust(
    y = park$loss_sum,
    x = park$dist_disc,
    covs = cbind(park$altura_tile_30arc, park$slope, park$roughness, park$prec, 
                 park$sq_1km.1, park$treecover_agg, park$clumps_1, as.factor(park$buffer_id)),
    vce = "nn",
    all = T
  )
})
...
setwd("~/Dropbox/BANREP/Backup Data/")
saveRDS(rd_robust_parks_2_ctrl_fx, "rd_robust_parks_2_ctrl_fx.rds")
saveRDS(rd_robust_terr_2_ctrl_fx, "rd_robust_terr_2_ctrl_fx.rds")
